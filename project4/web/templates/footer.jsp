<%-- 
    Document   : footer
    Created on : Apr 13, 2019, 11:03:08 AM
    Author     : johnn
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<div class="footer">
    <div class="footer-tu">
        <div class="container">
            <div class="footer-about col-md-3 col-sm-12 col-xs-12">
                <img src="https://cdn.shopify.com/s/files/1/2334/1307/files/shop_images_footer_e552736b-324e-47c1-90f3-23538bcc9f66_190x30.png?v=1507887912" alt="">
                <p>Morbi accumsan ipsum velit. Nam nec tellus a odio tincidunt auctor a ornare odio. Sed non mauris vitae
        Cerat consequat auctor eu in elit. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos.</p>
            </div>
            <div class="footer-account col-md-2 col-sm-12 col-xs-12">
                <h3>Account</h3>
                <ul>
                    <li><a>Wishlist</a></li>
                    <li><a>Your cart</a></li>
                    <li><a>Checkout</a></li>
                    <li><a>Login</a></li>
                </ul>
            </div>
            <div class="footer-information col-md-2 col-sm-12 col-xs-12">
                    <h3>Information</h3>
                    <ul>
                        <li><a>About us</a></li>
                        <li><a>Jobs</a></li>
                        <li><a>Sitemap</a></li>
                        <li><a>Contact</a></li>
                    </ul>
            </div>
            <div class="footer-quicklink col-md-2 col-sm-12 col-xs-12">
                <h3>QUICK LINKS</h3>
                <ul>
                    <li><a>Search</a></li>
                    <li><a>Delivery Information</a></li>
                    <li><a>Privacy Policy</a></li>
                    <li><a>Terms & Conditions</a></li>
                </ul>
            </div>
            <div class="footer-bip col-md-3 col-sm-12 col-xs-12">
                <iframe src="https://www.facebook.com/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2Ffptcorp%2F&tabs=timeline&width=292&height=200&small_header=false&adapt_container_width=true&hide_cover=false&show_facepile=true&appId" width="292" height="200" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>
            </div>
        </div>
    </div>
    <div class="footer-copyright">
        <span>&copy; Copyright 2019 CANGCUCOT v2 PRJ4.</span>
        <span>Powered by Thầy Nguyên</span>
    </div>
    
    <div id="abcxyz"><i class="fal fa-chevron-up"></i></div>
</div>