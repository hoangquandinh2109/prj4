...
public class changeStatusServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (purStatus == 1) {
                int quanSaudoi = product.getQuantity() - purchaseItem.getQuantity();
                int quancurrent = product.getQuantity();
                pur.setPurchaseStatus(purStatus);
                Date date = new Date();
                pur.setDateOrderPaid(date);
                purchaseFacade.edit(pur);
                product.setQuantity(quanSaudoi);
                productFacade.edit(product);
                request.setAttribute("quancurrent", quancurrent);
                request.getRequestDispatcher("showPurServlet").forward(request, response);
            } else if (purStatus == 2) {
                int quanSaudoi = product.getQuantity();
                pur.setPurchaseStatus(purStatus);
                Date date = new Date();
                pur.setDateOrderPaid(date);
                purchaseFacade.edit(pur);
